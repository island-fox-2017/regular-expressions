...
const grab_ktp = (string) => {
  let grab = string.match(/\d{3}-\d{2}-\d{4}/i);
  if (grab) {
    return grab[0];
  }
  else {
...
const grab_all_nomor_ktp = (string) => {
  let grab = string.match(/\d{3}-\d{2}-\d{4}/gi);
  if (grab) {
    return grab
  }
  else {
...
const hide_all_nomor_ktp = (string) => {
  ...
  let grab = string.match(/(\d{3})-(\d{2})-(\d{4})/gi);
  if (grab) {
    return string.replace(/(\d{3})-(\d{2})-(\d{4})/gi, 'XXX-XXX-$3');
  }
  else {
  return string
  }
...
const format_nomor = (string) => {
  let rep = string.replace(/[.-]/gi, '')
  let grab = string.match(/(\d{3})(\d{2})(\d{4})/gi);
  if (grab) {
    return rep.replace(/(\d{3})(\d{2})(\d{4})/gi, '$1-$2-$3');
  }
  else {
  return string
  }
