...
const grab_ktp = (string) => {
  var pattern = /(\d{3})-(\d{2})-(\d{4})/;
  var dapetinKtp = string.match(pattern);
  // console.log(dapetinKtp);
  if (dapetinKtp != null) {
    return dapetinKtp[0];
  } else {
    return null;
...
const hide_all_nomor_ktp = (string) => {
  var umpetinNomor = string.replace(/(\d{3})-(\d{2})-(\d{4})/g, 'XXX-XXX-$3');
  // console.log(umpetinNomor);
  if (umpetinNomor != null) {
    return umpetinNomor;
  } else {
    return ''
  }
...
const format_nomor = (string) => {
  var nomor = string.replace(/(\d{3})-(\d{2})-(\d{4})/g, '$1-$2-$3');
  nomor = nomor.replace(/\./g, '-');
  nomor = nomor.replace(/(\d{3})(\d{2})(\d{4})/, '$1-$2-$3');
  if(nomor != null) {
    return nomor;
  } else {
    return [];
